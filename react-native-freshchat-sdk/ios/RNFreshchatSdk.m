...
RCT_EXPORT_METHOD(setUser:(NSDictionary *)args :(RCTResponseSenderBlock)errorCallback)
{
    if (args == nil || ![args isKindOfClass:[NSDictionary class]]) {
            NSLog(@"Invalid arguments provided to setUser");
            [self postError:errorCallback :@"setUser" :@"Invalid arguments provided"];
            return;
    }
...
{
    if (args == nil || ![args isKindOfClass:[NSDictionary class]]) {
            NSLog(@"Invalid arguments provided to setUserProperties");
            [self postError:errorCallback :@"setUserProperties" :@"Invalid arguments provided"];
            return;
    }
    NSArray *keys = [args allKeys];
    if(keys.count == 0){
        ...
        if (arrayOfKeys.count != arrayOfValues.count) {
                NSLog(@"Number of keys and values in args dictionary do not match");
                [self postError:errorCallback :@"setUserProperties" :@"Mismatched keys and values"];
                return;
        }
        @try {
            NSString *key;
...
RCT_EXPORT_METHOD(setBotVariables:(NSDictionary *)variables :(NSDictionary *)botSpecificVariables)
{
    NSLog(@"Before native call");
    
//     NSDictionary *variables1 = @{
//   @"testBotProperty":@"Thirumal"};
//   NSDictionary *botSpecificVariables1 = @{
//    @"d57dd86f-85fb-4d4b-8a2e-ec39ada82589" : @{
//     @"testBotProperty":@"value"
// }};
NSLog(@"*%@",variables);
// NSLog(@"*%@",botSpecificVariables1);
    [[Freshchat sharedInstance] setBotVariables:variables withBotSpecificVariables:botSpecificVariables];
}
...
RCT_EXPORT_METHOD(showConversation:(NSString *)topicname withConversationReferenceID:(NSString *)conversationReferenceId)
{
    dispatch_async(dispatch_get_main_queue(), ^{
        UIViewController *viewController = [[[[UIApplication sharedApplication] delegate] window] rootViewController];
        [[Freshchat sharedInstance] showConversation:viewController withTopicName:topicname withConversationReferenceID:conversationReferenceId];
    });
}
