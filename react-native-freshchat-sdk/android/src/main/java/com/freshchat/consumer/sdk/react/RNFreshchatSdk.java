...
public class RNFreshchatSdk extends ReactContextBaseJavaModule {
    ...
    public void showConversations() {
        Log.e(LOG_TAG, "Show conversation in asdfasdf");
    ...
    public void setUserProperties(@NonNull ReadableMap readableMap, @Nullable final Callback errorCallback) {
        Log.e(LOG_TAG, "Please provide user properties to update the user - outside");
        if (readableMap == null) {
     ...
     @ReactMethod
    public void setBotVariables(@NonNull ReadableMap readableMapVariables, @Nullable ReadableMap botSpecificReadableMapVariables) {
        if (readableMapVariables == null) {
            Log.e(LOG_TAG, "Please provide valid bot Variables");
            return;
        }
        HashMap<String, Object> variables = readableMapVariables.toHashMap();
        HashMap<String, Map<String, Object>> botSpecificVariables = new HashMap<>();
        
        if (botSpecificReadableMapVariables != null && !botSpecificReadableMapVariables.toHashMap().isEmpty()) {
            HashMap<String, Object> botVariables = botSpecificReadableMapVariables.toHashMap();

            for (Map.Entry<String, Object> entry : botVariables.entrySet()) {
                String key = entry.getKey();
                HashMap<String, Object> value = (HashMap<String, Object>) entry.getValue();
                botSpecificVariables.put(key, value);
            }
        }
        Log.e(LOG_TAG, "Before native call");
        Freshchat.getInstance(getContext()).setBotVariables(variables, botSpecificVariables);
    }
    ...
    @ReactMethod
    public void showConversation(String topicName, String conversationReferenceId) {
        Freshchat.showConversations(getContext(), conversationReferenceId, topicName);
    }
